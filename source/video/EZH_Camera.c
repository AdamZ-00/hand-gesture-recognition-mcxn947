...
volatile uint16_t g_camera_ping_pong_buffer[CAMERA_WIDTH*STRIPE_HEIGHT*2]__attribute((aligned(4)));
...
void ezh_start(void)
{
	...
	smartdmaParam.p_buffer_ping_pong  		 = (uint32_t*)&g_camera_ping_pong_buffer[0];
	smartdmaParam.p_stripe_index  = (uint32_t*)&p_coord_index;
	SMARTDMA_Boot(EZH_Camera_384_384_Div16_Buf, &smartdmaParam, 0x2);
...
void ezh_camera_display_callback(void)
{
	...
	if((p_coord_index % 2) == 0){
		camera_buffer_addr = (uint32_t)(&g_camera_ping_pong_buffer[0]);

	}else{
		camera_buffer_addr = (uint32_t)(&g_camera_ping_pong_buffer[CAMERA_WIDTH*STRIPE_HEIGHT]);
	}

	ezh_copy_slice_to_model_input(p_coord_index,(uint32_t)(camera_buffer_addr),CAMERA_WIDTH,STRIPE_HEIGHT, CAMERA_HEIGHT/STRIPE_HEIGHT);
	display_show_slice(p_coord_index, (uint32_t)(camera_buffer_addr),CAMERA_HEIGHT/STRIPE_HEIGHT);
